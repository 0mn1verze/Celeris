...
impl SearchStack {
    pub(crate) fn at(&self, ply: u16, offset: i16) -> SearchStackEntry {
        assert!(ply as i16 + SEARCH_STACK_OFFSET as i16 - offset >= 0);
        self.stack[(ply as i16 + SEARCH_STACK_OFFSET as i16 - offset) as usize]
    }

    pub(crate) fn at_mut(&mut self, ply: u16, offset: i16) -> &mut SearchStackEntry {
        assert!(ply as i16 + SEARCH_STACK_OFFSET as i16 - offset >= 0);
        &mut self.stack[(ply as i16 + SEARCH_STACK_OFFSET as i16 - offset) as usize]
    }
}
